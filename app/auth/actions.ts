...
export async function signup(formData: FormData) {
  ...
  const { data: existingEmail } = await supabase.from("profiles").select("email").eq("email", email).maybeSingle()
  ...
  return { success: true, data: signUpData, message: "Sign up successful! Please check your email to confirm." }
}
...
export async function signin(formData: FormData) {
  ...
  if (!identifier || !password) {
    return { success: false, error: { message: "Email/Phone and password are required." } }
  }
  ...
  if (signInError) {
    return { success: false, error: { message: signInError.message || "Invalid credentials." } }
  }

  return { success: true, data: signInData, message: "Signed in successfully!" }
}
...
export async function sendResetPasswordEmail(formData: FormData) {
  ...
  return { success: true, message: "Please check your email to reset your password." }
}
...
export async function updatePassword(formData: FormData) {
  ...
  if (!password || password.length < 8 || password.length > 60) {
    return { success: false, error: { message: "Password must be 8-60 characters." } }
  }
